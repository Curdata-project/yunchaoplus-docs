...
| status       | string | 状态,默认是created。可能的值为：pending（进行中）, created（已创建）, failed（创建失败） |
...
| is_active          | string | 是否激活，必须为1（删除）                                    | required    |
| encrypt_pwd        | string | 密码密文（参见Client SDK文档）                               | required    |
| encrypt_type       | string | 加密类型，必须为1（删除）                                    | required    |
| plug_random_key    | string | 控件随机因子，通过“获取控件随机因子”接口获取，有效期为24小时。 | required    |
| mct_order_no       | string | 商户订单号（删除，钱包对象创建id）                           | optional    |
| rel_type           | string | 关联类型，留空（删除）                                       | optional    |
